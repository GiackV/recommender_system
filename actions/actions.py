...
class RestaurantAPI:
    ...
    def _getBoundsFromLatLng(self, lat, lng, halfSideInKm):
        # degrees to radians
        def Deg2rad(degrees):
            return float(math.pi) * float(degrees)/180
        ...
        bounds = { 
            "lat_min": Rad2deg(latMin),
            "lon_min": Rad2deg(lonMin),
            "lat_max": Rad2deg(latMax),
            "lon_max": Rad2deg(lonMax)
        }
        return bounds
    ...
    def _find_cuisine(self, slot_food):
        ...
        if slot_food is not None:
            for key, value in cuisine.items():
                if slot_food.lower() in key.lower():
                    food = value
...
class ActionSuggest(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker, domain):
        ...
        elif options and idx>0:
            place = tracker.get_slot("place")
            dispatcher.utter_message(text="Sorry, I didn't find any other restaurant in " + place + ". Wanna change something?")
            return []
        else:
            dispatcher.utter_message(text="I'm really sorry, for some reason the search did not return any results. Wanna change something?")
            return []
